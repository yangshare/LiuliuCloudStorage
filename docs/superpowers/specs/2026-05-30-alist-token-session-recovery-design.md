...
- 如果校验失败且无法恢复，返回认证过期结果，并通知渲染进程立即进入登录处理流程。
...
4. 如果恢复失败，将当前任务保留为 pending 或重新放回队列，发送一次 auth-failed 事件，不发送普通 download-failed 事件，并要求渲染进程立即跳转登录页。
...
- 如果 session 无效且无法刷新，队列保持暂停，并立即跳转登录页；登录页或跳转前通知显示“Alist 登录已过期，请重新登录后恢复下载”。
...
全局自动同步进度卡片应显示失败状态。自动刷新 token 失败时，应立即跳转登录页，而不是只等待用户点击失败卡片；失败卡片保留时也应指向登录页或会话恢复入口，而不是只跳到分享转存页。
...
自动刷新 token 失败后，应用应立即跳转登录页。该跳转不等待用户点击通知，因为失败原因可能是服务器认证接口异常、密码已修改、账号状态变化或本地保存的自动登录凭据不可用，需要用户主动处理。
...
- 自动刷新 token 失败时，触发渲染进程认证失效事件，用于立即跳转登录页。
...
- 如果自动登录无法刷新 token，应用立即跳转登录页，由用户处理重新登录、密码变更或服务器认证异常。
